...
void validate_options(const slp::Options &options) {
  if (options.ternary &&
    ...
    throw std::invalid_argument("--ternary is only supported with "
...
void fill_options(argparse::ArgumentParser &program, slp::Options &options) {
  ...
  if (search_method == "greedy_potential") {
    ...
    throw std::invalid_argument("received invalid argument for search method");
  }
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (options.ternary) {
      ...
      std::cout << "-------------------------------------------" << std::endl;
      std::cout << "Naive additions: " << result.additions_before << std::endl;
      std::cout << "After additions: " << result.additions_after << std::endl;
