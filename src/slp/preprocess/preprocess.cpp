    ...
    for (size_t i = 0; i < m; i++) {
        ...
        if (cnt == 0)
            row_col_type.push_back(
                {i,
                 std::numeric_limits<size_t>::max(), // can be anything
...
Z2Matrix column_preprocess(const Z2Matrix &G,
    ...
    for (size_t j = 0; j < n; j++) {
        ...
        if (pc == 0)
            row_col_type.push_back({std::numeric_limits<size_t>::max(), j,
                                    PreprocType::ColRemove0});
        else if (pc == 1)
            row_col_type.push_back(
...
preprocess(const Z2Matrix &G) {
    ...
    std::vector<PreprocStep> preproc_steps;
    size_t prev_m = std::numeric_limits<size_t>::max();
    size_t prev_n = std::numeric_limits<size_t>::max();
    Z2Matrix G_prime = G;
    ...
    while (prev_m != G_prime.m || prev_n != G_prime.n) {
        ...
        G_prime = row_preprocess(G, preproc_steps);
    ...
    while (!preproc_steps.empty()) {
        ...
        if (preproc_step.type == PreprocType::RowRemove0) {
            ...
            result.method.outputs.insert(result.method.outputs.begin() + row,
                                         col);
        } else if (preproc_step.type == PreprocType::RowRemove1) {
            // inserts before current row
            result.method.outputs.insert(result.method.outputs.begin() + row,
                                         col);
            ...
            result.method.outputs[row] = result.method.additions.size() - 1;
