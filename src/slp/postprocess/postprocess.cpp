...
bool temp_var_remover(const std::vector<uint64_t> &basis,
    ...
    for (size_t restore_idx : to_restore) {
        uint64_t s = basis.at(restore_idx);
        bool found = false;
        size_t restore_i = 0;
        size_t restore_j = 0;
...
AdditionMethod postprocess(const AdditionMethod &addition_method,
    ...
    validate_method(processed_addition_method, n, "postprocess start");
    ...
    while (can_be_removed) {
        ...
        validate_method(processed_addition_method, n, "postprocess before basis build");
        // remove single
